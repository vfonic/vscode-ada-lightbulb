    ...
    commitDetails(repo, commitHash) {
        return new Promise(resolve => {
            Promise.all([
                ...
                new Promise((resolve, reject) => this.execGit('diff-tree --name-status -r -m --root --find-renames --diff-filter=AMDR ' + commitHash, repo, (err, stdout) => {
                    if (err)
                        reject();
                    else
                        resolve(stdout.split(eolRegex));
                })),
                new Promise((resolve, reject) => this.execGit('diff-tree --numstat -r -m --root --find-renames --diff-filter=AMDR ' + commitHash, repo, (err, stdout) => {
                    if (err)
                        reject();
                    else
                        resolve(stdout.split(eolRegex));
                ...
                let details = results[0], fileLookup = {};
