...
(function() {
  ...
  var Dropdown = (function() {
              ...
              if (
                ...
                if (_this.selectedOption !== selectedOption) {
                  ...
                  _this.changeCallback(
                    _this.options[_this.selectedOption].value
                  );
                }
              }
            }
    ...
    Dropdown.prototype.render = function() {
      ...
      this.optionsElem.className =
        "dropdownOptions" + (this.showInfo ? " showInfo" : "");
      ...
      this.currentValueElem.style.width =
        Math.max(
          this.menuElem.offsetWidth +
            (this.showInfo && this.menuElem.offsetHeight < 272 ? 0 : 12),
          130
        ) + "px";
      this.menuElem.style.cssText =
        "right:0; overflow-y:auto; max-height:297px;";
  ...
  var Branch = (function() {
    ...
    Branch.prototype.draw = function(svg, config, expandAt) {
      var colour =
          config.graphColours[this.colour % config.graphColours.length],
        i,
        x1,
        y1,
        x2,
  ...
  var Vertex = (function() {
    ...
    Vertex.prototype.registerUnavailablePoint = function(
      x,
      connectsToVertex,
      onBranch
    ) {
    ...
    Vertex.prototype.draw = function(svg, config, expandOffset) {
      ...
      var colour = this.isCommitted
        ? config.graphColours[
            this.onBranch.getColour() % config.graphColours.length
          ]
        : "#808080";
      circle.setAttribute(
        "cx",
        (this.x * config.grid.x + config.grid.offsetX).toString()
      );
      circle.setAttribute(
        "cy",
        (
  ...
  var Graph = (function() {
      ...
      for (i = 0; i < commits.length; i++) {
        for (j = 0; j < commits[i].parentHashes.length; j++) {
          if (typeof commitLookup[commits[i].parentHashes[j]] === "number") {
            this.vertices[i].addParent(
              this.vertices[commitLookup[commits[i].parentHashes[j]]]
            );
          }
        }
      }
    ...
    Graph.prototype.determinePath = function(startAt) {
      ...
      var lastPoint = vertex.isNotOnBranch()
          ? vertex.getNextPoint()
          : vertex.getPoint(),
        curPoint;
      if (
        ...
        for (i = startAt + 1; i < this.vertices.length; i++) {
          curPoint = this.vertices[i].getPointConnectingTo(
            parentVertex,
            parentBranch
          );
          if (curPoint !== null) {
          ...
          this.vertices[i].registerUnavailablePoint(
            curPoint.x,
            parentVertex,
            parentBranch
          );
  ...
  var GitGraphView = (function() {
    function GitGraphView(repos, lastActiveRepo, config, prevState) {
      ...
      this.repoDropdown = new Dropdown("repoSelect", true, "Repos", function(
        value
      ) {
      ...
      this.showRemoteBranchesElem = document.getElementById(
    ...
    GitGraphView.prototype.loadBranches = function(
      branchOptions,
      branchHead,
      hard,
      isRepo
      ...
      if (
        this.currentBranch === null ||
        (this.currentBranch !== "" &&
          this.gitBranches.indexOf(this.currentBranch) === -1)
      ) {
        this.currentBranch =
    ...
    GitGraphView.prototype.triggerLoadBranchesCallback = function(
      changes,
      isRepo
    ) {
    ...
    GitGraphView.prototype.loadCommits = function(
      commits,
      commitHead,
      moreAvailable,
      hard
      ...
      for (i = 0; i < this.commits.length; i++) {
        ...
        if (
          this.expandedCommit !== null &&
          this.expandedCommit.hash === this.commits[i].hash
        )
    ...
    GitGraphView.prototype.requestLoadBranches = function(
      hard,
      loadedCallback
    ) {
    ...
    GitGraphView.prototype.renderGraph = function() {
      ...
      var headerHeight = colHeadersElem.clientHeight + 1,
        expandedCommitElem =
          this.expandedCommit !== null
            ? document.getElementById("commitDetails")
            : null;
    ...
    GitGraphView.prototype.renderTable = function() {
      ...
      var html = `
        ...
        i,
        currentHash =
          this.commits.length > 0 && this.commits[0].hash === "*"
            ? "*"
            : this.commitHead;
      for (i = 0; i < this.commits.length; i++) {
        var refs = "",
        ...
        html +=
          ...
          '"><td></td><td>' +
          refs +
          (this.commits[i].hash === currentHash
            ? "<b>" + message + "</b>"
            : message) +
          '</td><td title="' +
          date.title +
        ...
        if (elem === null) {
          ...
          if (
            ...
            this.showCommitDetails(
              this.expandedCommit.commitDetails,
        ...
        showContextMenu(
          ...
          [
            ...
            },
            {
              ...
              onClick: function() {
                if (
                  _this.commits[_this.commitLookup[hash]].parentHashes
                    .length === 1
                ) {
                  showConfirmationDialog(
                  ...
                  var options = _this.commits[
                    _this.commitLookup[hash]
                  ].parentHashes.map(function(hash, index) {
                    return {
                      name:
                        abbrevCommit(hash) +
      ...
      addListenerToClass("commit", "click", function(e) {
        ...
        if (
          _this.expandedCommit !== null &&
          _this.expandedCommit.hash === sourceElem.dataset.hash
        ) {
          // _this.hideCommitDetails();
        } else {
    ...
    GitGraphView.prototype.checkoutBranchAction = function(
      sourceElem,
      refName
    ) {
      var _this = this;
    ...
    GitGraphView.prototype.makeTableResizable = function() {
      ...
      for (var i = 0; i < cols.length; i++) {
        cols[i].innerHTML +=
          (i > 0
            ? '<span class="resizeCol left" data-col="' + (i - 1) + '"></span>'
            : "") +
          (i < cols.length - 1
            ? '<span class="resizeCol right" data-col="' + i + '"></span>'
            : "");
      }
      if (columnWidths !== null) {
        ...
        cols[0].style.padding =
          "0 " +
          Math.round(
            (Math.max(this.graph.getWidth() + 16, 64) -
              (cols[0].offsetWidth - 24)) /
              2
          ) +
          "px";
      }
    ...
    GitGraphView.prototype.hideCommitDetails = function() {
      if (this.expandedCommit !== null) {
        ...
        if (
          typeof this.expandedCommit.srcElem === "object" &&
    ...
    GitGraphView.prototype.showCommitDetails = function(
      commitDetails,
      fileTree
    ) {
      var _this = this;
      ...
      html +=
        "<b>Date: </b>" +
        new Date(commitDetails.date * 1e3).toString() +
        "<br>";
      html +=
      ...
      document
        .getElementById("commitDetailsClose")
        .addEventListener("click", function() {
          _this.hideCommitDetails();
        });
  ...
  function generateGitFileTreeHtml(folder, gitFiles) {
    ...
    keys.sort(function(a, b) {
      return folder.contents[a].type === "folder" &&
        folder.contents[b].type === "file"
        ? -1
        : folder.contents[a].type === "file" &&
          folder.contents[b].type === "folder"
        ? 1
        ...
        html +=
          ...
          (gitFile.type === "R"
            ? ' <span class="gitFileRename" title="' +
              escapeHtml(
                gitFile.oldFilePath + " was renamed to " + gitFile.newFilePath
              ) +
              '">R</span>'
            : "") +
          (gitFile.type !== "A" &&
  ...
  function showFormDialog(message, inputs, actionName, actioned, sourceElem) {
    ...
    var html =
      message +
      '<br><table class="dialogForm ' +
      (multiElementForm ? "multi" : "single") +
      '">';
    for (var i = 0; i < inputs.length; i++) {
      var input = inputs[i];
      html +=
        "<tr>" +
        (multiElementForm ? "<td>" + input.name + "</td>" : "") +
        "<td>";
      if (input.type === "select") {
        html +=
          '<select form="' + DIALOG_FORM_ID + '" id="dialogInput' + i + '">';
    ...
    showDialog(
      ...
      function() {
        if (
          ...
          dialog.className === "active inputInvalid"
        )
          return;
        var values = [];
        for (var i = 0; i < inputs.length; i++) {
  ...
  function showDialog(html, actionName, dismissName, actioned, sourceElem) {
    ...
    if (actionName !== null && actioned !== null) {
      ...
      document
        ...
        .addEventListener("click", actioned);
    }
    document
