...
(function() {
    ...
    function addListenerToClass(className, event, eventListener) {
        var elems = document.getElementsByClassName(className), i;
    ...
    var Dropdown = function() {
            ...
            document.addEventListener("click", function(e) {
                ...
                if (e.target === _this.currentValueElem) {
                    ...
                    if (e.target.closest(".dropdown") !== _this.elem) {
                        ...
                        if (option !== null && option.parentNode === _this.optionsElem && typeof option.dataset.id !== "undefined") {
        ...
        Dropdown.prototype.filter = function() {
            var val = this.filterInput.value.toLowerCase(), match, matches = false;
            for (var i = 0; i < this.options.length; i++) {
    ...
    var Branch = function() {
        ...
        Branch.prototype.draw = function(svg, config, expandAt) {
            var colour = config.graphColours[this.colour % config.graphColours.length], i, x1, y1, x2, y2, lines = [], curPath = "", curColour = "", d = config.grid.y * (config.graphStyle === "angular" ? .38 : .8);
        ...
        Branch.prototype.drawPath = function(svg, path, colour) {
            var line1 = document.createElementNS("http://www.w3.org/2000/svg", "path"), line2 = document.createElementNS("http://www.w3.org/2000/svg", "path");
    ...
    var Vertex = function() {
        ...
        Vertex.prototype.getPointConnectingTo = function(vertex, onBranch) {
            for (var i = 0; i < this.connections.length; i++) {
                if (this.connections[i].connectsTo === vertex && this.connections[i].onBranch === onBranch) return {
                    x: i,
                    y: this.y
                };
            }
        ...
        Vertex.prototype.draw = function(svg, config, expandOffset) {
            ...
            var colour = this.isCommitted ? config.graphColours[this.onBranch.getColour() % config.graphColours.length] : "#808080";
    ...
    var Graph = function() {
        function Graph(id, config) {
            ...
            var defs = document.createElementNS(svgNamespace, "defs"), linearGradient = document.createElementNS(svgNamespace, "linearGradient"), mask = document.createElementNS(svgNamespace, "mask");
        ...
        Graph.prototype.render = function(expandedCommit) {
            var group = document.createElementNS("http://www.w3.org/2000/svg", "g"), i, width = this.getWidth();
        ...
        Graph.prototype.determinePath = function(startAt) {
            var i = startAt;
            var vertex = this.vertices[i], parentVertex = this.vertices[i].getNextParent();
            var lastPoint = vertex.isNotOnBranch() ? vertex.getNextPoint() : vertex.getPoint(), curPoint;
            if (parentVertex !== null && vertex.isMerge() && !vertex.isNotOnBranch() && !parentVertex.isNotOnBranch()) {
                var foundPointToParent = false, parentBranch = parentVertex.getBranch();
    ...
    var GitGraphView = function() {
        ...
        GitGraphView.prototype.loadRepos = function(repos, lastActiveRepo) {
            this.gitRepos = repos;
            this.saveState();
            var repoPaths = Object.keys(repos), changedRepo = false;
        ...
        GitGraphView.prototype.loadBranches = function(branchOptions, branchHead, hard, isRepo) {
            ...
            if (!hard && arraysEqual(this.gitBranches, branchOptions, function(a, b) {
                return a === b;
        ...
        GitGraphView.prototype.loadCommits = function(commits, commitHead, moreAvailable, hard) {
            if (!hard && this.moreCommitsAvailable === moreAvailable && this.commitHead === commitHead && arraysEqual(this.commits, commits, function(a, b) {
                return a.hash === b.hash && arraysEqual(a.refs, b.refs, function(a, b) {
            ...
            this.commitLookup = {};
            this.saveState();
            var i, expandedCommitVisible = false, avatarsNeeded = {};
        ...
        GitGraphView.prototype.loadAvatar = function(email, image) {
            ...
            this.saveState();
            var avatarsElems = document.getElementsByClassName("avatar"), escapedEmail = escapeHtml(email);
        ...
        GitGraphView.prototype.renderGraph = function() {
            ...
            var headerHeight = colHeadersElem.clientHeight + 1, expandedCommitElem = this.expandedCommit !== null ? document.getElementById("commitDetails") : null;
        ...
        GitGraphView.prototype.renderTable = function() {
            var _this = this;
            var html = '<tr id="tableColHeaders"><th id="tableHeaderGraphCol" class="tableColHeader">Graph</th><th class="tableColHeader">Description</th><th class="tableColHeader">Date</th><th class="tableColHeader">Author</th><th class="tableColHeader">Commit</th></tr>', i, currentHash = this.commits.length > 0 && this.commits[0].hash === "*" ? "*" : this.commitHead;
            for (i = 0; i < this.commits.length; i++) {
                var refs = "", message = escapeHtml(this.commits[i].message), date = getCommitDate(this.commits[i].date), j = void 0, refName = void 0, refActive = void 0, refHtml = void 0;
            ...
            if (this.moreCommitsAvailable) {
                document.getElementById("loadMoreCommitsBtn").addEventListener("click", function() {
                    document.getElementById("loadMoreCommitsBtn").parentNode.innerHTML = '<h2 id="loadingHeader">' + svgIcons.loading + "Loading ...</h2>";
            ...
            if (this.expandedCommit !== null) {
                var elem = null, elems = document.getElementsByClassName("commit");
                for (i = 0; i < elems.length; i++) {
            ...
            addListenerToClass("commit", "contextmenu", function(e) {
                ...
                showContextMenu(e, [ {
                    ...
                    onClick: function() {
                        showRefInputDialog("Enter the name of the branch you would like to create from commit <b><i>" + abbrevCommit(hash) + "</i></b>:", "", "Create Branch", function(name) {
                            sendMessage({
                    ...
                    onClick: function() {
                        showConfirmationDialog("Are you sure you want to checkout commit <b><i>" + abbrevCommit(hash) + "</i></b>? This will result in a 'detached HEAD' state.", function() {
                            sendMessage({
                    ...
                    onClick: function() {
                        if (_this.commits[_this.commitLookup[hash]].parentHashes.length === 1) {
                            showConfirmationDialog("Are you sure you want to cherry pick commit <b><i>" + abbrevCommit(hash) + "</i></b>?", function() {
                                sendMessage({
                            ...
                            var options = _this.commits[_this.commitLookup[hash]].parentHashes.map(function(hash, index) {
                                return {
                                    name: abbrevCommit(hash) + (typeof _this.commitLookup[hash] === "number" ? ": " + _this.commits[_this.commitLookup[hash]].message : ""),
                                    value: (index + 1).toString()
                                };
                            });
                            showSelectDialog("Are you sure you want to cherry pick merge commit <b><i>" + abbrevCommit(hash) + "</i></b>? Choose the parent hash on the main branch, to cherry pick the commit relative to:", "1", options, "Yes, cherry pick commit", function(parentIndex) {
                                sendMessage({
                                    command: "cherrypickCommit",
                                    repo: _this.currentRepo,
                                    commitHash: hash,
                    ...
                    onClick: function() {
                        if (_this.commits[_this.commitLookup[hash]].parentHashes.length === 1) {
                            showConfirmationDialog("Are you sure you want to revert commit <b><i>" + abbrevCommit(hash) + "</i></b>?", function() {
                                sendMessage({
                            ...
                            showSelectDialog("Are you sure you want to revert merge commit <b><i>" + abbrevCommit(hash) + "</i></b>? Choose the parent hash on the main branch, to revert the commit relative to:", "1", options, "Yes, revert commit", function(parentIndex) {
                                sendMessage({
                                    command: "revertCommit",
                                    repo: _this.currentRepo,
                                    commitHash: hash,
                    ...
                    onClick: function() {
                        showCheckboxDialog("Are you sure you want to merge commit <b><i>" + abbrevCommit(hash) + "</i></b> into the current branch?", "Create a new commit even if fast-forward is possible", true, "Yes, merge", function(createNewCommit) {
                            sendMessage({
                                command: "mergeCommit",
                                repo: _this.currentRepo,
                                commitHash: hash,
            ...
            addListenerToClass("gitRef", "contextmenu", function(e) {
                ...
                var refName = unescapeHtml(sourceElem.dataset.name), menu, copyType;
                if (sourceElem.classList.contains("tag")) {
                    menu = [ {
                        title: "Delete Tag" + ELLIPSIS,
                        onClick: function() {
                            showConfirmationDialog("Are you sure you want to delete the tag <b><i>" + escapeHtml(refName) + "</i></b>?", function() {
                                sendMessage({
                                    command: "deleteTag",
                                    repo: _this.currentRepo,
                                    tagName: refName
                        ...
                        onClick: function() {
                            showConfirmationDialog("Are you sure you want to push the tag <b><i>" + escapeHtml(refName) + "</i></b>?", function() {
                                sendMessage({
                                    command: "pushTag",
                                    repo: _this.currentRepo,
                                    tagName: refName
                    ...
                    if (sourceElem.classList.contains("head")) {
                        ...
                        menu.push({
                            ...
                            onClick: function() {
                                showRefInputDialog("Enter the new name for branch <b><i>" + escapeHtml(refName) + "</i></b>:", refName, "Rename Branch", function(newName) {
                                    sendMessage({
                        ...
                        if (_this.gitBranchHead !== refName) {
                            menu.push({
                                ...
                                onClick: function() {
                                    showCheckboxDialog("Are you sure you want to delete the branch <b><i>" + escapeHtml(refName) + "</i></b>?", "Force Delete", false, "Delete Branch", function(forceDelete) {
                                        sendMessage({
                                ...
                                title: "Merge into current branch" + ELLIPSIS,
                                onClick: function() {
                                    showCheckboxDialog("Are you sure you want to merge branch <b><i>" + escapeHtml(refName) + "</i></b> into the current branch?", "Create a new commit even if fast-forward is possible", true, "Yes, merge", function(createNewCommit) {
                                        sendMessage({
                                            command: "mergeBranch",
                                            repo: _this.currentRepo,
                                            branchName: refName,
        ...
        GitGraphView.prototype.checkoutBranchAction = function(sourceElem, refName) {
            ...
            if (sourceElem.classList.contains("head")) {
                ...
                showRefInputDialog("Enter the name of the new branch you would like to create when checking out <b><i>" + escapeHtml(sourceElem.dataset.name) + "</i></b>:", refNameComps[refNameComps.length - 1], "Checkout Branch", function(newBranch) {
                    sendMessage({
                        command: "checkoutBranch",
                        repo: _this.currentRepo,
        ...
        GitGraphView.prototype.makeTableResizable = function() {
            var _this = this;
            var colHeadersElem = document.getElementById("tableColHeaders"), cols = document.getElementsByClassName("tableColHeader");
            var columnWidths = this.gitRepos[this.currentRepo].columnWidths, mouseX = -1, col = -1, that = this;
        ...
        GitGraphView.prototype.observeWindowSizeChanges = function() {
            var _this = this;
            var windowWidth = window.outerWidth, windowHeight = window.outerHeight;
        ...
        GitGraphView.prototype.showCommitDetails = function(commitDetails, fileTree) {
            ...
            this.saveState();
            var newElem = document.createElement("tr"), html = '<td></td><td colspan="4"><div id="commitDetailsSummary">';
    ...
    var contextMenu = document.getElementById("contextMenu"), contextMenuSource = null;
    var dialog = document.getElementById("dialog"), dialogBacking = document.getElementById("dialogBacking"), dialogMenuSource = null;
    var gitGraph = new GitGraphView(viewState.repos, viewState.lastActiveRepo, {
    ...
    function getCommitDate(dateVal) {
        var date = new Date(dateVal * 1e3), value;
        ...
        switch (viewState.dateFormat) {
          ...
          case "Relative":
            var diff = Math.round(new Date().getTime() / 1e3) - dateVal, unit = void 0;
            if (diff < 60) {
                unit = "second";
            } else if (diff < 3600) {
                unit = "minute";
    ...
    function generateGitFileTreeHtml(folder, gitFiles) {
        var html = (folder.name !== "" ? '<span class="gitFolder" data-folderpath="' + encodeURIComponent(folder.folderPath) + '"><span class="gitFolderIcon">' + (folder.open ? svgIcons.openFolder : svgIcons.closedFolder) + '</span><span class="gitFolderName">' + folder.name + "</span></span>" : "") + '<ul class="gitFolderContents' + (!folder.open ? " hidden" : "") + '">', keys = Object.keys(folder.contents), i, gitFile, gitFolder;
        keys.sort(function(a, b) {
            return folder.contents[a].type === "folder" && folder.contents[b].type === "file" ? -1 : folder.contents[a].type === "file" && folder.contents[b].type === "folder" ? 1 : folder.contents[a].name < folder.contents[b].name ? -1 : folder.contents[a].name > folder.contents[b].name ? 1 : 0;
        });
    ...
    function alterGitFileTree(folder, folderPath, open) {
        var path = folderPath.split("/"), i, cur = folder;
    ...
    function showFormDialog(message, inputs, actionName, actioned, sourceElem) {
        var textRefInput = -1, multiElementForm = inputs.length > 1;
        ...
        html += "</table>";
        showDialog(html, actionName, "Cancel", function() {
            if (dialog.className === "active noInput" || dialog.className === "active inputInvalid") return;
            var values = [];
            for (var i = 0; i < inputs.length; i++) {
                var input = inputs[i], elem = document.getElementById("dialogInput" + i);
                if (input.type === "select") {
        ...
        if (textRefInput > -1) {
            var dialogInput_1 = document.getElementById("dialogInput" + textRefInput), dialogAction_1 = document.getElementById("dialogAction");
            if (dialogInput_1.value === "") dialog.className = "active noInput";
            dialogInput_1.focus();
            dialogInput_1.addEventListener("keyup", function() {
                var noInput = dialogInput_1.value === "", invalidInput = dialogInput_1.value.match(refInvalid) !== null;
